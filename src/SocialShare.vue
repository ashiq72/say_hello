...
<script setup>
import { defineComponent, h, computed, shallowRef } from "vue";
...
const shareLink = computed(() => {
  ...
  if (key.value === "twitter") {
    if (!props.share.hashtags.length) link = link.replace("&hashtags=@h", "");
    if (!props.share.twitterUser.length) link = link.replace("@tu", "");
  }
  return link
    .replace(/@tu/g, "&via=" + encodeURIComponent(props.share.twitterUser))
    .replace(/@u/g, encodeURIComponent(props.share.url))
    .replace(/@t/g, encodeURIComponent(props.share.title))
    .replace(/@d/g, encodeURIComponent(props.share.description))
    .replace(/@q/g, encodeURIComponent(props.share.quote))
    .replace(/@h/g, encodedHashtags.value)
    .replace(/@m/g, encodeURIComponent(props.share.media));
});
const encodedHashtags = computed(() => {
  if (key.value === "facebook" && props.share.hashtags.length) {
    return "%23" + this.hashtags.split(",")[0];
  }

  return props.share.hashtags;
});
const shareUrl = (item) => {
  key.value = item;
